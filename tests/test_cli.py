...
import onzr
from onzr.cli import ExitCodes
from onzr.config import SETTINGS_FILES
...
def test_init_command_without_input(runner, onzr_cli, settings_files):
    ...
    result = runner.invoke(onzr_cli, ["init"])
    assert result.exit_code == SYSTEM_EXIT_1

    # Base configuration exists by withour ARL setting
    for setting_file in settings_files:
        assert setting_file.exists() is True

    # Dist file and its initial copy hould be identical
    for src, dest in zip(SETTINGS_FILES, settings_files, strict=True):
        dist = Path(onzr.__file__).parent / Path(f"{src}.dist")
        assert dist.exists()
        assert dist.read_text() == dest.read_text()


def test_init_command(runner, onzr_cli, settings_files):
    """Test the `onzr init` command."""
    for setting_file in settings_files:
        assert setting_file.exists() is False

    result = runner.invoke(onzr_cli, ["init"], input="fake-arl")
    assert result.exit_code == ExitCodes.OK
...
def test_init_command_does_not_overwrite_settings(runner, onzr_cli, settings_files):
...
def test_init_command_reset(runner, onzr_cli, settings_files):
    ...
    result = runner.invoke(onzr_cli, ["init", "--reset"], input="new-fake-arl")
    assert result.exit_code == ExitCodes.OK
